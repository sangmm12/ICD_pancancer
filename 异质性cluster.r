...
out_put_list <- c()
file_name = 'F:/ICD/yzx/cluster'
final_tumor_list = c()

my_data = read.csv('F:/ICD/data/yzx.csv',header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  p <- ggboxplot(dat, x = "Gene", y = "value",
                color = "Group", palette = c('#FF6600','#009D91','#9fbdb0','#ffc93c'),
