...
tumor_list = c("ACC","BLCA","BRCA","CESC","CHOL","COAD","ESCA","GBM","HNSC","KICH","KIRC","KIRP","LAML","LGG","LIHC","LUAD","LUSC","OV","PAAD","PCPG","PRAD","READ","SKCM","STAD","TGCT","THCA","UCEC","UCS")
...
for (tumor_id in tumor_list)
{
  ...
  dat_input <- read.csv("F:/ICD/data/gx.csv",check.names = F,header=T)
  ...
  pdf(paste('gx/corr/',tumor_id,".pdf",sep=''),width = 15,height = 5)
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +  # 使用点表示数�?
    scale_color_gradient2(low = "#96CDCD", high = "#8B4789") +  # 调整颜色渐变
...
##拼图
pdf("fig1.pdf",width = length(colnames(dat_input))*12,height = length(colnames(dat_one_tumor))*length(out_put_list)/30)
