...
for (tumor_id in tumor_list)
{
  ...
  dat_input <- read.csv("F:/ICD/data/yzx.csv",check.names = F,header=T)
  ...
  pdf(paste('yzx/corr/',tumor_id,".pdf",sep=''),width = 15,height = 5)
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +  # 使用点表示数�?
    scale_color_gradient2(low = '#BEBEBE', high = '#8B6914') +  # 调整颜色渐变,
