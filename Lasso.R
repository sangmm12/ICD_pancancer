...
Tumor <- unique(gsub('\\([^\\)]*\\)','',read.csv('F:/ICD/data/ICD_tpm_exp.csv')$CODE))

final_list=data.frame()

for(name_fold in Tumor)
{
  all_sur_data <- read.csv('F:/ICD/data/survival.csv',header=T,check.names=F)
  all_sur_data <- all_sur_data[grep(name_fold,all_sur_data$CODE),][,-c(1)]
  
  gene_exp <- read.csv('F:/ICD/data/ICD_tpm_exp.csv')
  ...
  for (i in 1:length(genes)) {
    ...
    lines(log(fit$lambda), coef_values, col = i + 1, lwd = 2)  # 使用不同的颜�?
    gene_label_x <- min(log(fit$lambda))
    gene_label_y <- ifelse(sum(coef_values) <= 0, min(coef_values), max(coef_values))
    text(gene_label_x+(abs(min(coef(fit))) + abs(max(coef(fit))))/500, gene_label_y+(abs(min(coef(fit))) + abs(max(coef(fit))))/500, labels = genes[i], col = i + 1,cex=0.7)  # 在最大系数点上添加基因标�?
  }
