...
other_data = read.csv("F:/ICD/cluster/out.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("F:/ICD/CIBER/cluster/",name,".pdf",sep=''),width=length(unique(dat[,1]))+3,height = 10)
...
######
pdf(paste("F:/ICD/CIBER/cluster/fig1.pdf",sep=''),width = length(colnames(exp_file))*8,height = 8*length(out_put_list)/3.5)
