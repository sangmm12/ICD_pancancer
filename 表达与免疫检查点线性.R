...
for(classify in data_list)
{
  ...
  #file_name = paste0('immune_check/',classify,'/corr')
 ...
 # print("Dir already exists!")
  ...
  for (tumor_id in tumor_list)
  {
    ...
    dat_input <- read.csv(paste0('E:/ICD/data/immune_check/',classify,'.csv'),header=T,check.names=F)
    ...
    data_all_tumor <- read.csv(paste("E:/ICD/data/ICD_tpm_exp.csv",sep=''),check.names = F)
    ...
    pdf(paste('immune_check/',classify,'/corr/',"/",tumor_id,".pdf",sep=''),width = 15,height = round(length(colnames(dat_input))/2.87,0))
    p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
      geom_point() +  # 使用点表示数�?
      scale_color_gradient2(low = '#29c6cd', high = '#CD6600') +  # 调整颜色渐变
  ...
  ##拼图
  pdf(paste('immune_check/',classify,'/corr/',"/","fig1.pdf",sep=''),width = 56,height = length(out_put_list)*round(length(colnames(dat_input))/2.87,0)/4)
