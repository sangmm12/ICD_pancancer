...
out_put_list <- c()
file_name = 'F:/ICD/gx/cluster'
final_tumor_list = c()

my_data = read.csv('F:/ICD/data/gx.csv',header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  p <- ggboxplot(dat, x = "Gene", y = "value",
                color = "Group", palette = c("#C46F0F","#262E47","#810528","#377F05"),
