...
data_list = c('Immunoinhibitor','Immunostimulator')
...
for(classify in data_list)
{
  ...
  my_data = read.csv(paste0('F:/ICD/data/immune_reg/',classify,'.csv'),header=T,check.names=F)
  ...
  for(name in tumor_list)
  {
    ...
    #中间凹陷
    p <- ggboxplot(dat, x = "Gene", y = "value",
                   fill = "Group", palette  = c('#f8b500','#623448','#34495e','#59057b'),
