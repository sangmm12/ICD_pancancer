...
my_data = read.csv('F:/ICD/data/CIBER.csv',header=T,check.names=F)

other_data = read.csv("F:/ICD/Lasso/risk.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("F:/ICD/CIBER/risk/",name,".pdf",sep=''),width=length(unique(dat[,1]))+3,height = 8)
  ...
  #中间凹陷
  p <- ggboxplot(dat, x = "Gene", y = "value",
                 fill = "Group", palette = c('#FF0000','#00CC00','#FFD300','#3914AF'),
...
######
pdf(paste("F:/ICD/CIBER/risk/fig1.pdf",sep=''),width = length(colnames(exp_file))*10,height =10*length(out_put_list)/3.5)
